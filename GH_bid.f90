...
program GHbid
    ...
    sm = 86
    cg = 99
    open(sm, file='smedio_32_xv_0,8_h.dat')
    open(cg, file='cluster_gigante_32_xv_0,8_h.dat')
    ...
    do kk=1,Npuntos
      ...
      T = T + DT
    enddo
!esta parte que sigue es para hacer loops de histéresis, comentar si no es necesario usar
    do kk=1,Npuntos
      do jj=1,20                     
        call dinamicaGH
      end do 
...
!-------------------------------------------------------------------------------------------------
      subroutine WS
        ...
        do z_n=0,N-1
          ...
          do i=1,rem
            sitiorem = 1
            do while((sitiorem == 1).or.(AA(z_i,sitiorem) == 0))  
              sitiorem=int(random()*N)
            end do
            i_l = sitiorem/L            
            j_l = mod(sitiorem,L)
            d_i = abs(i_n-i_l)                     !Calcula las distancias entre pares de nodos considerando condiciones
            d_j = abs(j_n-j_l)                     !periodicas de borde en las d_i(j)_per
            d_i_per = mod(abs(i_n-i_l),L)
            d_j_per = mod(abs(j_n-j_l),L)   !calcular las distancias para cada nodo que desconecte, analizar que orden de vecino
                                            !es y en funcion de eso deconectar su nodo simetrico   
            if (((d_i==1) .and. (d_j==0)) .or. ((d_i==0) .and. (d_j==1)) .or. ((d_i_per==L-1) .and. (d_j==0)) &
            .or. ((d_i==0) .and. (d_j_per==L-1))) then
              if (d_i==1) then 
